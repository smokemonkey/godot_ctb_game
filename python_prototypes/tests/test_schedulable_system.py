...
class TestSchedulableSystem(unittest.TestCase):
    ...
    def test_ctb_manager_add_schedulable(self):
        ...
        retrieved = self.ctb_manager.get_schedulable("test_actor")
    ...
    def test_ctb_manager_remove_schedulable(self):
        ...
        removed = self.ctb_manager.remove_schedulable("test_actor")
    ...
    def test_ctb_manager_execute_schedulable(self):
        ...
        self.ctb_manager.execute_schedulable(actor)
    ...
    def test_ctb_manager_set_active(self):
        ...
        result = self.ctb_manager.set_schedulable_active("test_actor", False)
        self.assertTrue(result)
        self.assertFalse(actor.is_active)
        
        result = self.ctb_manager.set_schedulable_active("test_actor", True)
        self.assertTrue(result)
        self.assertTrue(actor.is_active)
    ...
    def test_ctb_manager_status_info(self):
        ...
        info_list = self.ctb_manager.get_schedulable_info()
    ...
    def test_mixed_scheduling_system(self):
        ...
        while processed_count < 3 and self.mock_calendar.current_time < 200:
            if not self.time_wheel._is_current_slot_empty():
                due = self.ctb_manager.get_due_schedulable()
                if due is not None:
                    self.ctb_manager.execute_schedulable(due)
                    processed_count += 1
            else:
                self.mock_calendar.advance_time_tick()
                self.time_wheel.advance_wheel()
