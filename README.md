...
- **小时级精度** 的时间控制
- **非匀速时间流逝** - 可按需推进时间
- **高精度时间**: 内部以小时为最小单位，支持超长时间跨度（数万年）的模拟
- **纪元与年号**: 支持自定义年号系统，如"建元"、"元狩"
...
### 2. CTB战斗系统 (ctb_manager) - v2.0 环形缓冲区版本
- **全新设计**: 基于环形缓冲区的高效事件调度
...
> - **起始时间**: 以公元前2000年作为时间起点
> - **核心数据结构**: 使用带索引的哈希时间轮 (`IndexedTimeWheel`)
> - **事件调度**:
...
# 4. 设置和使用年号
time_manager.set_era_anchor("大汉", -140)  # 设置"大汉"年号，起始于公元前140年
print(time_manager.get_current_era_name())
print(time_manager.get_current_era_year())