...
func before_each():
	...
	time_wheel = IndexedTimeWheel.new(180, mock_calendar.get_timestamp)
...
func test_ctb_manager_initialization():
	...
	# 检查是否有事件被安排
	var upcoming = time_wheel.peek_upcoming_events(50, 10)
	assert_gt(upcoming.size(), 0, "Should have scheduled events after initialization")
...
func test_ctb_manager_execute_schedulable():
	...
	# 设置执行回调
	ctb_manager.on_event_executed = func(schedulable):
		executed_count += 1
